...
/**
 * Local copy of order-service's RedissonConfig — same workaround, reused
 * rather than rediscovered.
 *
 * Builds the RedissonClient bean by hand — a stopgap, not the intended
 * long-term design. Redisson's own Spring Boot starter auto-configuration
 * (RedissonAutoConfigurationV2, explicitly excluded via
 * spring.autoconfigure.exclude in application.yml) is currently broken
 ...
 * Left enabled, that auto-configuration throws ClassNotFoundException the
 * moment it runs, failing context refresh for the entire application — not
 * just anything Redisson-related.
 *
